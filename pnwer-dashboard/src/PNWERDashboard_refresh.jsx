...
// ═══════════════════════════════════════════════════════════════
// Static display constants (unchanged by refresh)
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// Fallback data (shown until first API load succeeds)
// These are exactly your original hardcoded values — the app
// works even if the backend is unreachable on first load.
// ═══════════════════════════════════════════════════════════════
const FALLBACK_US = [
...
const FALLBACK_YEARLY = [
  { year: "2017", pnwer: 38.7 }, { year: "2018", pnwer: 42.9 }, { year: "2019", pnwer: 44.5 },
  { year: "2020", pnwer: 36.6 }, { year: "2021", pnwer: 52.0 }, { year: "2022", pnwer: 66.8 },
  { year: "2023", pnwer: 62.5 }, { year: "2024", pnwer: 58.8 }, { year: "2025", pnwer: 48.0 },
];

const FALLBACK_NATIONAL_YEARLY = [
  { year: "2017", usmca: 1138.1, control: 604.8 }, { year: "2018", usmca: 1228.0, control: 458.7 },
  { year: "2019", usmca: 1224.2, control: 672.3 }, { year: "2020", usmca: 1062.2, control: 592.2 },
  { year: "2021", usmca: 1326.6, control: 688.4 }, { year: "2022", usmca: 1575.5, control: 775.7 },
  { year: "2023", usmca: 1569.0, control: 779.3 }, { year: "2024", usmca: 1601.3, control: 808.3 },
  { year: "2025", usmca: 1457.8, control: 749.8 },
];

const FALLBACK_DID_DATA = [
  { period: "2017", treated: 100.0, control: 100.0 }, { period: "2018", treated: 107.9, control: 75.9 },
  { period: "2019", treated: 107.6, control: 111.2 }, { period: "2020", treated: 93.4, control: 97.9 },
  { period: "2021", treated: 116.6, control: 113.8 }, { period: "2022", treated: 138.4, control: 128.3 },
  { period: "2023", treated: 137.9, control: 128.9 }, { period: "2024", treated: 140.7, control: 133.7 },
  { period: "2025", treated: 128.1, control: 124.0 },
];

const FALLBACK_DDD_DATA = [
...
const FALLBACK_SUMMARY = {
  totalTrade2024B: 58.8, totalTrade2025B: 48.0, tradeLossB: -10.9, tradeLossPct: -18.5,
  gdpAtRiskM: 13156.3, jobsAtRisk: 81799, tariffEffectB: -6.6, tariffSharePct: 60,
  oilEffectB: -2.9, oilSharePct: 26, residualB: -1.4, residualSharePct: 13,
};

// ═══════════════════════════════════════════════════════════════
// Data transformer
// Converts pnwer_analysis_data_v9.json → dashboard display shape.
// Only updates US state cards and time-series charts —
// Canadian estimates and model coefficients are not in the Census
// data so they remain as fallbacks.
// ═══════════════════════════════════════════════════════════════
function transformApiData(raw) {
  const st = raw?.state_trade;
  const nat = raw?.national_trade;
  if (!st || !nat) return null;

  const PNWER_STATES = ["WA", "OR", "ID", "MT", "AK"];
  const TOP_EXPORTS = {
    WA: "Aircraft & Spacecraft", OR: "Integrated Circuits",
    ID: "Wheat & Agriculture",   MT: "Crude Petroleum", AK: "Minerals & Ores",
  };
  const IND_MAP = { agriculture: "Agriculture", manufacturing: "Manufacturing", energy: "Energy", forestry: "Forestry" };

  // ── US jurisdiction cards ──────────────────────────────────────────────────
  const US = PNWER_STATES.map((id) => {
    const stateData = st[id];
    let totalTrade24 = 0, totalJobs = 0;
    const sectors = { Agriculture: 0, Manufacturing: 0, Energy: 0, Forestry: 0 };

    for (const partner of ["CA", "MX"]) {
      const y24 = stateData?.[partner]?.["2024"];
      if (!y24) continue;
      totalTrade24 += (y24.total?.exports ?? 0) + (y24.total?.imports ?? 0);
      for (const [apiKey, displayKey] of Object.entries(IND_MAP)) {
        sectors[displayKey] += ((y24.by_industry?.[apiKey]?.exports ?? 0) +
                                (y24.by_industry?.[apiKey]?.imports ?? 0)) / 1e9;
      }
      // Rough jobs estimate: $1M trade ≈ 5 jobs
      totalJobs += ((y24.total?.exports ?? 0) + (y24.total?.imports ?? 0)) / 1e6 * 5;
    }
    // Round sector values to 2 dp
    Object.keys(sectors).forEach(k => { sectors[k] = Math.round(sectors[k] * 100) / 100; });

    return {
      id,
      name: { WA: "Washington", OR: "Oregon", ID: "Idaho", MT: "Montana", AK: "Alaska" }[id],
      flag: "🇺🇸",
      trade: Math.round(totalTrade24 / 1e9 * 10) / 10,
      topExport: TOP_EXPORTS[id],
      gdpImpact: FALLBACK_US.find(j => j.id === id)?.gdpImpact ?? 0,  // model output, not in raw data
      jobs: Math.round(totalJobs),
      sectors,
    };
  });

  // ── PNWER yearly totals ────────────────────────────────────────────────────
  const years = raw.metadata?.years ?? [];
  const YEARLY = years.map((yr) => {
    let total = 0;
    for (const id of PNWER_STATES) {
    for (const partner of ["CA", "MX"]) {
      const y = st[id]?.[partner]?.[String(yr)];
      if (!y) continue;
      total += (y.total?.exports ?? 0) + (y.total?.imports ?? 0);
    }
  }
  return { year: String(yr), pnwer: Math.round(total / 1e9 * 10) / 10 };
});

  // ── National yearly (USMCA vs control) ────────────────────────────────────
  const CONTROL_CODES = ["CN", "JP", "DE", "UK", "KR"];
  const NATIONAL_YEARLY = years.map((yr) => {
    const usmcaCA = nat?.CA?.years?.[String(yr)];
    const usmcaMX = nat?.MX?.years?.[String(yr)];
    const usmca = ((usmcaCA?.total_trade ?? 0) + (usmcaMX?.total_trade ?? 0)) / 1e9;
    const control = CONTROL_CODES.reduce((sum, c) => {
      return sum + (nat?.[c]?.years?.[String(yr)]?.total_trade ?? 0) / 1e9;
    }, 0);
    return { year: String(yr), usmca: Math.round(usmca * 10) / 10, control: Math.round(control * 10) / 10 };
  });

  // ── Summary stats ──────────────────────────────────────────────────────────
  const total24 = US.reduce((s, j) => s + j.trade, 0);
  const total25 = (() => {
    let t = 0;
    for (const id of PNWER_STATES) {
      for (const partner of ["CA", "MX"]) {
        const y = st[id]?.[partner]?.["2025"];
        if (!y) continue;
        t += (y.total?.exports ?? 0) + (y.total?.imports ?? 0);
      }
    }
    return Math.round(t / 1e9 * 10) / 10;
  })();

  const tradeLoss = Math.round((total25 - total24) * 10) / 10;
  const SUMMARY = {
    ...FALLBACK_SUMMARY,
    totalTrade2024B: Math.round(total24 * 10) / 10,
    totalTrade2025B: total25,
    tradeLossB: tradeLoss,
    tradeLossPct: Math.round(tradeLoss / total24 * 1000) / 10,
    jobsAtRisk: US.reduce((s, j) => s + j.jobs, 0),
  };

  // ── DDD state bar data ─────────────────────────────────────────────────────
  const DDD_DATA = PNWER_STATES.map((id) => {
    let t24 = 0, t25 = 0;
    for (const partner of ["CA", "MX"]) {
      const y24 = st[id]?.[partner]?.["2024"]?.total;
      const y25 = st[id]?.[partner]?.["2025"]?.total;
      t24 += (y24?.exports ?? 0) + (y24?.imports ?? 0);
      t25 += (y25?.exports ?? 0) + (y25?.imports ?? 0);
    }
    const fallback = FALLBACK_DDD_DATA.find(d => d.id === id);
    return {
      id,
      current:   Math.round(t24 / 1e9 * 10) / 10,
      postModel: fallback?.postModel ?? 0,  // model output, preserved from fallback
      actual25:  Math.round(t25 / 1e9 * 10) / 10,
    };
  });

  return { US, YEARLY, NATIONAL_YEARLY, DDD_DATA, SUMMARY };
}

// ═══════════════════════════════════════════════════════════════
// Refresh hook — encapsulates all polling logic
// ═══════════════════════════════════════════════════════════════
function useRefresh(onComplete) {
  const [phase, setPhase]       = useState("idle");   // idle|starting|running|success|partial|error
  ...
  const pollStatus = useCallback(async () => {
    try {
      const data = await fetch(`${API_BASE}/api/refresh/status`).then(r => r.json());
      if (data.progress) setProgress({ step: data.progress.step ?? "", percent: data.progress.percent ?? 0 });
      if (!data.running && data.last_result) {
        stopPolling();
        setLastResult(data.last_result);
        setPhase(data.last_result.status);
        if (data.last_result.status === "success" || data.last_result.status === "partial") {
          onComplete?.();
        }
      }
  ...
  const trigger = async () => {
    setPhase("starting");
    setProgress({ step: "Connecting to Census Bureau API…", percent: 0 });
    try {
      ...
      setPhase("error");
      setLastResult({ status: "error", errors: [err.message] });
    }
  };

  useEffect(() => () => stopPolling(), []);

  return { phase, progress, lastResult, trigger };
}
...
export default function PNWERDashboard() {
  ...
  // ── Live data state (starts as null; fallbacks used until loaded) ──────────
  const [liveData, setLiveData]     = useState(null);
  const [dataLoading, setDataLoading] = useState(false);
  const [lastUpdated, setLastUpdated] = useState(null);

  // ── Fetch fresh data from /api/data and transform it ──────────────────────
  const fetchLiveData = useCallback(async () => {
    setDataLoading(true);
    try {
      const raw  = await fetch(`${API_BASE}/api/data`).then(r => r.json());
      const transformed = transformApiData(raw);
      if (transformed) {
        setLiveData(transformed);
        setLastUpdated(raw.metadata?.last_refreshed ?? raw.metadata?.generated_at ?? null);
      }
    } catch {
      // Silently fall through — fallback data keeps the dashboard working
    } finally {
  ...
  // ── Refresh hook ──────────────────────────────────────────────────────────
  const { phase: refreshPhase, progress, lastResult, trigger } = useRefresh(fetchLiveData);

  // ── Resolved data: live if available, fallback otherwise ──────────────────
  const US           = liveData?.US              ?? FALLBACK_US;
  const ALL          = [...US, ...FALLBACK_CA];
  const YEARLY       = liveData?.YEARLY          ?? FALLBACK_YEARLY;
  const NATIONAL_YEARLY = liveData?.NATIONAL_YEARLY ?? FALLBACK_NATIONAL_YEARLY;
  const DDD_DATA     = liveData?.DDD_DATA        ?? FALLBACK_DDD_DATA;
  const SUMMARY      = liveData?.SUMMARY         ?? FALLBACK_SUMMARY;
  const HS           = FALLBACK_HS;
  const DID_DATA     = FALLBACK_DID_DATA;   // model coefficients don't change with raw data

  const isRefreshing = refreshPhase === "starting" || refreshPhase === "running";
  ...
  // ── Refresh status banner (shown at top of every tab while running) ───────
  const RefreshBanner = () => {
        const [visible, setVisible] = useState(true);

  useEffect(() => {
    if (lastResult && !isRefreshing) {
      const timer = setTimeout(() => setVisible(false), 4000); // closes after 4 seconds
      return () => clearTimeout(timer);
    }
  }, [lastResult, isRefreshing]);

  if (!visible || (refreshPhase === "idle" && !lastResult)) return null;

    if (refreshPhase === "idle" && !lastResult) return null;
    return (
      <div style={{
        position: "fixed", bottom: 20, right: 24, zIndex: 200,
        background: "rgba(10,37,64,0.96)", borderRadius: 14, padding: "14px 18px",
        border: "1px solid rgba(1,186,239,0.2)", boxShadow: "0 8px 32px rgba(0,0,0,0.3)",
        minWidth: 280, maxWidth: 340,
      }}>
        {isRefreshing ? (
          <>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#01BAEF",
                animation: "pulse 1.5s infinite" }} />
              <span style={{ color: "white", fontSize: 12, fontWeight: 600 }}>Refreshing {new Date().getFullYear()} data…</span>
            </div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", marginBottom: 8, height: 16 }}>
              {progress.step}
            </div>
            <div style={{ background: "rgba(255,255,255,0.1)", borderRadius: 100, height: 4, overflow: "hidden" }}>
              <div style={{ height: "100%", borderRadius: 100, background: "linear-gradient(90deg,#01BAEF,#20BF55)",
                width: `${progress.percent}%`, transition: "width 0.5s" }} />
            </div>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", marginTop: 6, textAlign: "right" }}>
              {progress.percent}% · Dashboard stays live during refresh
            </div>
          </>
        ) : lastResult && (
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 18 }}>
              {lastResult.status === "success" ? "✅" : lastResult.status === "partial" ? "⚠️" : "❌"}
            </span>
            <div>
              <div style={{ color: "white", fontSize: 12, fontWeight: 600 }}>
                {lastResult.status === "success" && "Data refreshed successfully"}
                {lastResult.status === "partial" && `Refreshed with ${lastResult.errors?.length} warning(s)`}
                {lastResult.status === "error"   && "Refresh failed"}
              </div>
              {lastResult.timestamp && (
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", marginTop: 2 }}>
                  {new Date(lastResult.timestamp).toLocaleString(undefined,
                    { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                  {lastResult.month_refreshed && ` · Census month ${lastResult.month_refreshed}/${new Date().getFullYear()}`}
                </div>
              )}
            </div>
          </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              {lastUpdated && !isRefreshing && (
                <div style={{ fontSize: 9, color: "rgba(255,255,255,0.3)", letterSpacing: 0.5 }}>
                  · Updated {new Date(lastUpdated).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}
                </div>
              )}
              {dataLoading && (
                <div style={{ fontSize: 9, color: "#01BAEF", letterSpacing: 0.5 }}>· Loading…</div>
              )}
            </div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button
            ...
            style={{
              ...
              padding: "7px 14px", borderRadius: 8, border: "1px solid rgba(1,186,239,0.3)",
              background: isRefreshing ? "rgba(1,186,239,0.05)" : "rgba(1,186,239,0.1)",
              color: isRefreshing ? "rgba(1,186,239,0.4)" : "#01BAEF",
          ...
          >
            {isRefreshing ? (
              <svg style={{ width: 12, height: 12, animation: "spin 1s linear infinite" }} fill="none" viewBox="0 0 24 24">
                ...
                <path fill="currentColor" opacity="0.75" d="M4 12a8 8 0 018-8v4l3-3-3-3v4A10 10 0 002 12h2z"/>
              </svg>
            ) : (
            ...
            {isRefreshing ? `${progress.percent}%` : `Refresh ${new Date().getFullYear()} Data`}
          </button>
      ...
      <div style={{ maxWidth: 1300, margin: "0 auto", padding: "28px 28px 56px" }}>
        ...
        {tab === "overview" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 10, marginBottom: 18 }}>
              {FALLBACK_CA.map(j => <JurCard key={j.id} j={j} />)}
            </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
                  ...
                  <div style={{ background: "white", borderRadius: 14, border: "1px solid #E4EAF0", padding: 24 }}>
                    ...
                    <div style={{ background: "rgba(32,191,85,0.08)", border: "1px solid rgba(32,191,85,0.15)", borderRadius: 8, padding: 12, marginTop: 14 }}>
                      <div style={{ fontSize: 12, fontWeight: 600, color: "#0A2540", marginBottom: 4 }}>Key Insight</div>
                      <div style={{ fontSize: 12, color: "#5A6B7C", lineHeight: 1.6 }}>PNWER states benefited disproportionately from USMCA — meaning they also face disproportionate risk from tariff disruption.</div>
                    </div>
                  </div>
                </div>
              ...
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
                  <div style={{ background: "white", borderRadius: 14, border: "1px solid #E4EAF0", padding: 24 }}>
                    ...
                    <div style={{ fontSize: 13, color: "#5A6B7C", lineHeight: 1.6, marginBottom: 14 }}>Total observed decline: ${Math.abs(SUMMARY.tradeLossB)}B</div>
                    {[
